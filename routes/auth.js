  ...
  try {
    const result = await pool.query(
      `SELECT u.*, s.name as site_name, s.contract_demand_kva, s.max_users
       FROM users u JOIN sites s ON s.id = u.site_id
    ...
    const token = jwt.sign(
      { id: u.id, username: u.username, site_id: u.site_id, role: u.role },
      process.env.JWT_SECRET || 'venora-jwt-secret-2025',
      { expiresIn: '24h' }
    );
...
router.post('/add-user', checkApiKey, async (req, res) => {
  ...
  try {
    const countResult = await pool.query(`SELECT COUNT(*) as count FROM users WHERE site_id = $1 AND active = true`, [site_id]);
    const siteResult  = await pool.query(`SELECT COALESCE(max_users, 2) as max_users FROM sites WHERE id = $1`, [site_id]);
    const currentCount = parseInt(countResult.rows[0].count);
    const maxUsers     = parseInt(siteResult.rows[0]?.max_users || 2);
    if (currentCount >= maxUsers)
      return res.status(403).json({ error: `User limit reached. Max ${maxUsers} users for this site.`, current_users: currentCount, max_users: maxUsers });
    await pool.query(
      `INSERT INTO users (username, password, site_id, display_name, company_name, role) VALUES ($1,$2,$3,$4,$5,$6)`,
...
router.get('/users', checkApiKey, async (req, res) => {
  ...
  try {
    const result     = await pool.query(`SELECT username, display_name, company_name, role, active, created_at FROM users WHERE site_id = $1 ORDER BY created_at ASC`, [site_id]);
    const siteResult = await pool.query(`SELECT COALESCE(max_users, 2) as max_users FROM sites WHERE id = $1`, [site_id]);
    res.json({ users: result.rows, count: result.rows.length, max_users: siteResult.rows[0]?.max_users || 2 });
...
router.post('/set-user-limit', checkApiKey, async (req, res) => {
  ...
  try {
    await pool.query(`UPDATE sites SET max_users = $1 WHERE id = $2`, [max_users, site_id]);
